...
each of which hands objects from one `osgeo` extension to another over the one shared GDAL.
Every row on screen is a residual or an exception class, not a claim.
...
- **A round trip that stays inside one extension.**
  ...
  — every one of those resolves to `_gdal`, so this panel has no cross-extension handoff in
  it. It prints how many of the 262,144 elements differ and the worst absolute residual.
- **Which extensions the import actually mapped.** The header reads the six `osgeo`
  extensions out of `sys.modules` before anything else runs, and again after
  `gdal.UseExceptions()`. Expect `4/6` on the first line and `6/6` on the second, on both
  platforms. The bytes beside them are summed from whichever of those extension files the
  loader can name on disk, and cover the SWIG wrappers only, not the shared `libgdal`.
...
- **An EPSG code beside the proj-string.**
  [`ImportFromEPSG(4326)`](https://gdal.org/en/stable/api/python/spatial_ref_api.html#osgeo.osr.SpatialReference.ImportFromEPSG)
  needs PROJ's database. iOS gets it from `flet-libproj` with no configuration; Android gets
  it only from `pyproj`, which is why `pyproject.toml` lists `pyproj` under
  `[tool.flet.android]` and extracts it with `extract_packages`. Drop either and that row
  prints a `RuntimeError` instead of a CRS name. The raster itself carries no CRS:
  georeferencing here is the affine transform (`gdal.InvGeoTransform` →
  `gdal.ApplyGeoTransform`), which needs no database at all.
...
desktop figures below came from a hand-built environment with GDAL on the host, and differ
in two places over and above the timings: the driver line reads hundreds rather than eleven,
and the extension byte totals are whatever your host GDAL happens to weigh. Every residual
should be identical. Against a host GDAL 3.13.0 the whole screen came out as 0 of 262,144 elements
